...
def calculate_kinetic_energy_of_particles(particles: Particles) -> float64:
    ...
    return sum( .5 * ptcl.mass * norm(ptcl.velocity) for ptcl in particles )


def calculate_potential_energy_of_particles(particles: Particles) -> float64:
    ...
    for chosen, other in ordered_pairs_permutations(particles):
        total_potential -= CFG.G * chosen.mass * other.mass / distances[(chosen.id, other.id)]
    return total_potential


def calculate_total_energy_of_particles(particles: Particles) -> float64:
...
def print_gravitational_boundedness(particles: Particles) -> None:
